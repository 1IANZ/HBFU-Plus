...
function parseCourseSchedule(html: string): CourseSchedule[] {
    ...
    Array.from(table.querySelectorAll("tr"))
        ...
        .forEach((row) => {
            ...
            const sectionNum =
                SECTION_NAMES.findIndex((cn) =>
                    sectionText.includes(`第${cn}大节`),
                ) + 1;
            ...
            Array.from(row.querySelectorAll("td"))
                ...
                .forEach((cell, dayIndex) => {
                    ...
                    const courseName = courseDiv.innerHTML
                        .split("<br>")[0]
                        .trim();
                    const weeks = extractText(
                        courseDiv,
                        'font[title*="周次"]',
                    ).replace(/\(周\)/g, "周");
                    const classroom = extractText(
                        courseDiv,
                        'font[title*="教室"]',
                    );

                    const detailDiv = cell.querySelector(".kbcontent");
                    const teacher = extractText(
                        detailDiv,
                        'font[title*="老师"]',
                    );
